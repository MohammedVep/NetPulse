...
3) creates a dedicated IAM deploy user with AssumeRole permissions for CDK roles,
...
QUALIFIER="hnb659fds"
DEPLOY_ROLE_ARN="arn:aws:iam::${ACCOUNT_ID}:role/cdk-${QUALIFIER}-deploy-role-${ACCOUNT_ID}-${REGION}"
FILE_ROLE_ARN="arn:aws:iam::${ACCOUNT_ID}:role/cdk-${QUALIFIER}-file-publishing-role-${ACCOUNT_ID}-${REGION}"
IMAGE_ROLE_ARN="arn:aws:iam::${ACCOUNT_ID}:role/cdk-${QUALIFIER}-image-publishing-role-${ACCOUNT_ID}-${REGION}"
LOOKUP_ROLE_ARN="arn:aws:iam::${ACCOUNT_ID}:role/cdk-${QUALIFIER}-lookup-role-${ACCOUNT_ID}-${REGION}"
